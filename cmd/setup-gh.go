...
func setSubscriptionID () string {
	...
	prompt := promptui.Prompt{
		Label:    "Enter Azure subscription ID",
		Validate: validate,
	}
...
func setResourceGroupName () string {
	validate := func(input string) error {
		if input == "" {
...
func (sc *SetUpCmd) InitializeSetUpConfig() (*SetUpCmd, error){
	sc.appName = setAppName()
	sc.subscriptionID = setSubscriptionID()
	sc.resourceGroupName = setResourceGroupName()
