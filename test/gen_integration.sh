...
do 
    ...
    create_config_args="--variable PORT=8080 --variable APPNAME=testingCreateCommand --variable VERSION=1.18 --variable SERVICEPORT=8080 --variable NAMESPACE=testNamespace --variable IMAGENAME=testImage --variable IMAGETAG=latest"
      ...
      steps:
        ...
        - name: Execute Dry Run with variables passed through flag 
          run: |
            mkdir -p test/temp
            ./draft --dry-run --dry-run-file test/temp/dry-run.json \
            create -d ./langtest/ --skip-file-detection -deploy-type helm \
            $create_config_args
        - name: Validate JSON
          run: |
            npm install -g ajv-cli@5.0.0
    ...
    steps:
      ...
      - name: Execute Dry Run with variables passed through flag 
        run: |
          mkdir -p test/temp
          ./draft --dry-run --dry-run-file test/temp/dry-run.json \
          create -d ./langtest/ --skip-file-detection -deploy-type kustomize \
          $create_config_args
      - name: Validate JSON
        run: |
          npm install -g ajv-cli@5.0.0
      ...
      steps:
        ...
        - name: Execute Dry Run with variables passed through flag 
          run: |
            mkdir -p test/temp
            ./draft --dry-run --dry-run-file test/temp/dry-run.json \
            create -d ./langtest/ --skip-file-detection -deploy-type manifests \
            $create_config_args
        - name: Validate JSON
          run: |
            npm install -g ajv-cli@5.0.0
